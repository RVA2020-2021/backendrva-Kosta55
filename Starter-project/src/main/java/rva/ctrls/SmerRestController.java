...
public class SmerRestController {
	...
	@GetMapping("smer")
	@ApiOperation(value="Vraæa kolekciju svih smerova iz baze podataka")
	public Collection<Smer> getSmerovi(){
		return smerRepository.findAll();
	}
	@GetMapping("smer/{id}")
	@ApiOperation(value="Vraæa smer na osnovu prosledjenog ID-ja iz baze podataka")
	public Smer getSmer(@PathVariable("id") Integer id) {
		return smerRepository.getOne(id);
	}
	@GetMapping("smerNaziv/{naziv}")
	@ApiOperation(value="Vraæa kolekciju smerova na osnovu naziva iz baze podataka")
